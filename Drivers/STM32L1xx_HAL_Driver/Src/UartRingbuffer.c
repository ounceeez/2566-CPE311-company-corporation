...
/* Remove the following HAL_UART_HandleTypeDef declaration */
// UART_HandleTypeDef huart2;

/* Uncomment the following line to use LL_USART */
#define uart USART2
...
extern void Uart_isr (USART_TypeDef *USARTx);
...
void Uart_write(int c)
{
	if (c>=0)
	{
		...
		SET_BIT(uart->CR1, USART_CR1_TXEIE); // Enable USART transmission interrupt
	}
}
...
void Uart_isr (USART_TypeDef *USARTx)
{
    ...
    if (((isrflags & USART_SR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET))
    {
    	 /******************
    	    	      ...
    	    	      *          sequence: a read operation to USART_ISR register followed by a read
    	    	      *          operation to USART_RDR register.
    	      ...
    	      *********************/
		USARTx->SR;                       /* Read status register */
        unsigned char c = USARTx->DR;     /* Read data register */
